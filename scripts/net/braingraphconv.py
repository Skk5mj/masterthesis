...
class MyNNConv(MyMessagePassing):
    def __init__(
        self, in_channels, out_channels, nn, normalize=False, bias=True, **kwargs
    ):
    ...
    def forward(self, x, edge_index, edge_weight=None, pseudo=None, size=None):
        ...
        # pseudoはROIの位置を示すベクトル。dataset.data.posに対応
        # このweightは論文の式(3)に対応しており、
        ...
        if torch.is_tensor(x):
            ...
            x = (
                None
                if x[0] is None
                else torch.matmul(x[0].unsqueeze(1), weight).squeeze(1),
                None
                if x[1] is None
                else torch.matmul(x[1].unsqueeze(1), weight).squeeze(1),
            )
